...
function DetailsForm({
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      setAddressud(userProfile.address);
      onFullName(userProfile.full_name);
      onPhone(userProfile.phone);
      onAddressud(userProfile.address);
    }
  }, [userProfile]);
  ...
  const handleAddressudChange = (event) => {
    setAddressud(event.target.value);
    onAddressud(event.target.value);
  };
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        ...
        <form onSubmit={userProfile ? onUpdate : onSubmit}>
          <TextField
            label="Full Name"
            type="text"
            fullWidth
          ...
          {userProfile ? (
            <TextField
              ...
              required
              value={addressud}
              onChange={handleAddressudChange}
            />
          ) : (
            <div>
